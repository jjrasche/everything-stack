...
// ============ Intent Adaptation State ============

class IntentAdaptationState extends BaseEntity {
  @override
  int id = 0;
  ...
  // ============ Intent tunable parameters ============

  /// Overall confidence threshold for intent classification
  /// Learned from: feedback on misclassified intents
  double confidenceThreshold = 0.65;

  /// Per-tool confidence thresholds
  /// Example: {"reminder": 0.60, "message": 0.70}
  /// Some tools are harder to detect accurately
  Map<String, double> toolConfidenceThresholds = {};

  /// Slot priority (which slots matter most)
  /// Learned from: which slots users correct most often
  /// Higher priority slots get more careful validation
  ...
  Map<String, dynamic> toJson() => {
    ...
    'deviceId': deviceId,
    'confidenceThreshold': confidenceThreshold,
    'toolConfidenceThresholds': toolConfidenceThresholds,
  ...
  factory IntentAdaptationState.fromJson(Map<String, dynamic> json) {
    final state = IntentAdaptationState(
      scope: json['scope'] as String,
      userId: json['userId'] as String?,
      deviceId: json['deviceId'] as String?,
    );
    ...
    state.syncId = json['syncId'] as String?;
    state.confidenceThreshold = json['confidenceThreshold'] as double? ?? 0.65;
    state.toolConfidenceThresholds = Map<String, double>.from(
        json['toolConfidenceThresholds'] as Map? ?? {});
    state.slotPriority =
        List<String>.from(json['slotPriority'] as List? ?? []);
    state.version = json['version'] as int? ?? 0;
    state.lastUpdatedAt = json['lastUpdatedAt'] != null
        ? DateTime.parse(json['lastUpdatedAt'] as String)
        : DateTime.now();
