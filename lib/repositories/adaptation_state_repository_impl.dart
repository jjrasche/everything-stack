...
class IntentAdaptationStateRepositoryImpl
    extends AdaptationStateRepository<IntentAdaptationState> {
  final Map<String, IntentAdaptationState> _store = {};

  IntentAdaptationStateRepositoryImpl._();

  factory IntentAdaptationStateRepositoryImpl.inMemory() {
    return IntentAdaptationStateRepositoryImpl._();
  }

  @override
  Future<IntentAdaptationState> getCurrent({String? userId}) async {
    if (userId != null) {
      try {
        return _store.values.firstWhere(
          (s) => s.scope == 'user' && s.userId == userId,
  ...
  @override
  Future<IntentAdaptationState?> getUserState(String userId) async {
    try {
      return _store.values.firstWhere(
        (s) => s.scope == 'user' && s.userId == userId,
      );
  ...
  @override
  Future<IntentAdaptationState?> getGlobal() async {
    return _store.values.firstWhere(
      (s) => s.scope == 'global',
      orElse: () => null as IntentAdaptationState,
    ) as IntentAdaptationState?;
  }

  @override
  Future<bool> updateWithVersion(IntentAdaptationState state) async {
    final current = _store[state.uuid];
    if (current == null || current.version != state.version) {
      return false;
    }
  ...
  @override
  Future<IntentAdaptationState> save(IntentAdaptationState state) async {
    _store[state.uuid] = state;
    return state;
  }

  @override
  Future<List<IntentAdaptationState>> getHistory() async {
    final states = _store.values.toList();
    states.sort((a, b) => a.version.compareTo(b.version));
    return states;
  }
  ...
  @override
  IntentAdaptationState createDefault() {
    return IntentAdaptationState(scope: 'global');
  }
