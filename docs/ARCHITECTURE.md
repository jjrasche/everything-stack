...
| Layer | Test File | What It Tests | Platforms |
|-------|-----------|---------------|-----------|
| Unit | `test/services/` | Service logic, algorithms, mocks | Dart VM |
| Integration | `test/integration/` | Cross-service workflows | Dart VM |
| BDD | `test/scenarios/` | User-facing behavior (Gherkin) | Dart VM |
| Platform | `integration_test/` | Actual platform code | Android, iOS, Web, Desktop |

Each layer must pass before merge. All tests run in CI.
...
- **Decision**: UUID as primary key
- **Rationale**: Universal across offline devices, no ID coordination, matches modern API design
- **Trade-off**: Slightly larger indices (negligible with modern databases)

### Adapter-as-Repository vs ORM
- **Decision**: Adapters extend repository interface (adapter pattern)
- **Rationale**: Domain entities stay ORM-free for web compilation, platform details isolated, easy to add new backends
- **Trade-off**: Manual serialization in wrappers (but automated by adapters)

### Dual Persistence (ObjectBox + IndexedDB)
- **Decision**: Native uses ObjectBox, web uses IndexedDB
- **Rationale**: ObjectBox superior performance, HNSW, transactions. IndexedDB only option on web. Same API via adapters.
- **Trade-off**: Two persistence implementations to maintain

### Trainable Mixin Pattern
- **Decision**: Generic `Trainable` mixin for feedback loops
- **Rationale**: Decouples feedback from component implementation, enables feedback on any entity, shared interface
- **Trade-off**: Requires adapters to implement training queries

### Turn Entity Inclusion
- **Decision**: Keep Turn as atomic feedback boundary
- **Rationale**:
  - Feedback collected at Turn level (user rates entire exchange, not components)
  - Training scoped to Turn (failure is systemic, not isolated)
  - Latency tracking (Turn.latencyMs captures end-to-end, not just component time)
  - Feedback queue (markedForFeedback is clear at Turn level)
  - Query patterns (show failed turns, slow turns, etc.)
- **Alternative (rejected)**: Flatten to Event+Invocation only (loses atomic feedback boundary, makes queries harder, fragments training context)
...
Add conflict resolution for AdaptationState:
- Timestamps + last-write-wins for simple merging
- CRDT library (e.g., Automerge) for complex state
- Supabase Realtime or polling for subscription

### Team Collaboration (v3)
Implement RLS + multi-user:
- Supabase Auth integration
- Row-level policies per user
- Shared conversations with ownership

### Edge Deployment (v4)
Deploy Supabase regionally:
- Reduce latency for geographic distribution
- Local-first sync, eventually consistent

### Advanced Vector Search (v4)
GraphQL API for remote semantic search:
- Hybrid ranking (semantic + recency)
...
- **ASD Workflow**: `docs/asd/WORKFLOW.md` - How to build features
- **Testing**: `docs/testing/TESTING_APPROACH.md` - 4-layer testing philosophy
- **Patterns**: `lib/patterns/README.md` - Pattern usage guide
- **Bootstrap**: `lib/bootstrap.dart` - Service initialization
- **FAQ**: `docs/ARCHITECTURE_FAQ.md` - Common questions
- **Persistence**: `lib/persistence/README.md` - Adapter implementation guide

---

**Last Updated**: Phase 6 Complete (Method Signature Alignment)
**Next Phase**: Phase 9 - Verify compilation and integration tests
